...
class _DineInTablesScreenContentState extends State<_DineInTablesScreenContent> {
  ...
  TableModel? sourceTableForTransfer; // 🔥 Store source table here
  ...
  void _startTransferMode(TableModel sourceTable) {
    setState(() {
      isTransferMode = true;
      sourceTableForTransfer = sourceTable; // 🔥 Store the source table
    });
  ...
  void _cancelTransferMode() {
    setState(() {
      ...
      sourceTableForTransfer = null; // 🔥 Clear source table
    });
  }
    ...
    if (state is GetTableOrderSuccess) {
      ...
      if (cartIds.isNotEmpty) {
        final success = await cubit.transferOrder(
          sourceTableId: sourceTable.id, // 🔥 Pass source table ID
          destinationTableId: destinationTable.id, // 🔥 Pass destination table ID
          cartIds: cartIds,
          newStatus: "available", // 🔥 Change source table status to available after transfer
        );
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isAvailableForTransfer = isTransferMode &&
                            (uiStatus == 'available') &&
                            (sourceTableForTransfer?.id != table.id); // Can't transfer to same table

                        return GestureDetector(
                          onTap: () {
                            if (isTransferMode) {
                              if (isAvailableForTransfer) {
                                _showTransferConfirmationDialog(sourceTableForTransfer!, table);
                              } else if (sourceTableForTransfer?.id == table.id) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text('Cannot transfer to the same table'),
                                    backgroundColor: Colors.orange,
                                    behavior: SnackBarBehavior.floating,
                                  ),
                                );
